...
<div>
    ...
    <b-row class="map-header">
        ...
        <b-col lg="2" offset-lg="2">
            <DatePicker lang="en" v-model="datepickerText" :format="datepicketFormat" range input-class="form-control" width="220">
    ...
    <b-row class="widget-middle-overflow">
        <div class="export-button">
            <download-excel class="btn btn-outline-default" :data="vehicles" worksheet="RouteAnalysis" type="csv" :fields="excel_fields" name="RouteAnalysis.csv">
                EXPORT EXCEL
            </download-excel>
        </div>
...
import rowVehicleData from 'C:/Users/Salman Saleem K M A/Downloads/MOCK_DATA.json';
...
export default {
    ...
    methods: {
        ...
        filterByOptions() {
            // console.log(this.vehicles[0]['plate_number'])
            if (this.search_input == '') {
                this.vehicles = rowVehicleData;
                this.clear();

            } else {
                this.getShippingDeparture();
                console.log(this.datepickerText);
                this.vehicles = rowVehicleData.filter((vehicle) => {
                    return vehicle[this.search_selected] != null &&
                        vehicle[this.search_selected].includes(this.search_input) //&&
                    // this.datepickerText[0] <= new Date(vehicle.departure_date) &&
                    // new Date(vehicle.departure_date) >= this.datepickerText[1];
                });
                if (this.datepickerText[0] != null && this.datepickerText[1] != null) {
                    this.vehicles = this.vehicles.filter(vehicle => this.datepickerText[0] <= new Date(vehicle.arrival_date) && this.datepickerText[1] >= new Date(vehicle.arrival_date));
                }
                // console.log(this.datepickerText[0] <= new Date(this.vehicles[0].departure_date) && this.datepickerText[1] >= new Date(this.vehicles[0].departure_date));
            }
        },
        clear() {
            this.search_selected = 'PlateNo';
