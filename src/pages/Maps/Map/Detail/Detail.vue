            ...
            <div v-if="!defaultPage" class="widget-body mt-3 detail-body" > 
                <div class="table-resposive" v-for="row in trailerDetail" :key="row.plateNo">    
                    ...
                    <div class="playback-btn">
                        <b-button v-if="role !== 'DEALER'" block variant="outline-danger" @click="goToPlayback()">PLAYBACK</b-button>  
                    </div> 
    ...
    methods: {
        toggle(plateNo) {
            ...
            if(typeof plateNo !== 'undefined'){
                ...
                .then((response) => {
                    ...
                    const trailer = response.data.resultBody;
                    this.trailerDetail = trailer;

                    for(let i in response.data.resultBody){
                        this.paramDepDate = trailer[i].shippingDetails[i].departureDate;
                    }   
                })
                .catch((ex)=> {
                    console.log(ex);
        ...
        goToPlayback(){
            this.$root.$router.push({
                name: 'PlaybackPage', 
                params: { plateNo: this.paramPlateNo , depDate: this.paramDepDate }
            });  
        },
        checkRole(){
